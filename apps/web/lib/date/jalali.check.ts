...
import { parseJalali, weekdayIndex, weekdayName } from "./jalali.ts"
...
assert.equal(parseJalali("۱۴۰۵٫۰۵٫۰۵"), "2026-07-27", "glyph separator")
...
// ── WEEKDAY INDEX ──────────────────────────────────────────
// Saturday is 0. 2026-09-11 is a Friday — index 6, the last day of the week.

assert.equal(weekdayIndex(new Date(2026, 8, 5)), 0, "saturday")
assert.equal(weekdayIndex(new Date(2026, 8, 11)), 6, "friday")
assert.equal(weekdayName(new Date(2026, 8, 11)), "جمعه", "friday name")
